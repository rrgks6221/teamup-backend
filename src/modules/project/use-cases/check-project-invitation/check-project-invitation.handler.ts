...
@CommandHandler(CheckProjectInvitationCommand)
export class CheckProjectInvitationHandler
  implements ICommandHandler<CheckProjectInvitationCommand, ProjectInvitation>
{
  constructor(
  ...
  async execute(
    command: CheckProjectInvitationCommand,
    ...
    if (invitation.inviteeId !== command.currentUserId) {
      throw new ProjectInvitationChangeStatusRestrictedError(
        'Only project invitee can check invitation status',
      );
    }

    project.markInvitationAsChecked(invitation);
