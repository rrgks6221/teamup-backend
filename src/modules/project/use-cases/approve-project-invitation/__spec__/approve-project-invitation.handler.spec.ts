...
import { ApproveProjectInvitationCommandFactory } from '@module/project/use-cases/approve-project-invitation/__spec__/approve-project-invitation-command.factory';
...
describe(ApproveProjectInvitationHandler.name, () => {
  let handler: ApproveProjectInvitationHandler;

  let projectRepository: ProjectRepositoryPort;
  let projectInvitationRepository: ProjectInvitationRepositoryPort;
  let eventStore: IEventStore;

  let command: ApproveProjectInvitationCommand;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<ApproveProjectInvitationHandler>(
      ApproveProjectInvitationHandler,
    );
  ...
  beforeEach(() => {
    command = ApproveProjectInvitationCommandFactory.build();
  ...
  describe('식별자와 일치하는 프로젝트가 존재하고', () => {
    ...
    describe('식별자와 일치하는 초대장이 존재하고', () => {
      ...
      describe('지원자 본인이 승인하는 경우', () => {
        describe('초대장을 승인하면', () => {
          it('초대장이 승인돼야한다.', async () => {
      ...
      describe('초대받지 않은 사람이 승인하려는 경우', () => {
        describe('초대장을 승인하면', () => {
          it('초대받은 사람만 초대장를 승인할 수 있다는 에러가 발생해야한다.', async () => {
            await expect(
              handler.execute({
                ...command,
